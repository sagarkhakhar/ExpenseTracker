...
/// Use case for updating an existing financial goal.
...
class UpdateFinancialGoal {
  ...
  UpdateFinancialGoal(this.repository);

  /// Call this to update a financial goal. Returns either a Failure or void on success.
  Future<Either<Failure, void>> call(FinancialGoal goal) async {
    // Business rule: Goal ID must not be empty
    if (goal.id.trim().isEmpty) {
      return const Left(ValidationFailure('Goal ID cannot be empty'));
    }
    ...
    final existingGoalResult = await repository.getFinancialGoalById(goal.id);
    return existingGoalResult.fold(
      (failure) => Left(failure),
      (existingGoal) {
        if (existingGoal == null) {
          return const Left(ValidationFailure('Financial goal not found'));
        }
        // If all validations pass, call the repository to update the goal
        return repository.updateFinancialGoal(goal);
      },
    );
  }
}
